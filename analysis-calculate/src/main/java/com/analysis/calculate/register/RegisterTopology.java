...
public class RegisterTopology {
	public static void main(String[] args) throws Exception {
		...
		SpoutConfig spoutConfig = new SpoutConfig(brokerHosts, TopicType.REGISTER.getTopic(), NginxConstants.KAFKA_ZKROOT_PREFIX+TopicType.REGISTER.getTopic(), spoutId);
		spoutConfig.scheme = new SchemeAsMultiScheme(new StringScheme());

		TopologyBuilder builder = new TopologyBuilder();
		builder.setSpout(spoutId, new KafkaSpout(spoutConfig));
		builder.setBolt(parserBolt, new RegisterParserBolt()).shuffleGrouping(spoutId);
		builder.setBolt(filterBolt, new RegisterFilterBolt()).shuffleGrouping(parserBolt);
		builder.setBolt(persistentBolt, new RegisterPersistentBolt()).shuffleGrouping(filterBolt);
