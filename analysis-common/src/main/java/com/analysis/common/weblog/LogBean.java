...
//    private String request;
    ...
    public void put(String field, Object value) {
        if(REMOTE_ADDR.equals(field)) {
            ...
            //TODO 加入ip位置
            String addinfo = IPMetadataParser.getCountry(remote_addr);
            String[] addinfoarr = addinfo.split(":");
            if(addinfoarr != null && addinfoarr.length == 3) {
                ipcountry = addinfoarr[0];
                ipprovince = addinfoarr[1];
                ipcity = addinfoarr[2];
            }
    ...
    public String buildString() {
        ...
        sb.append(ipcountry).append('\t');
        sb.append(ipprovince).append('\t');
        sb.append(ipcity).append('\t');
        sb.append(uri == null ? "" : uri).append('\t');
        sb.append(status == null ? "" : status).append('\t');
        sb.append(time_local).append('\t');
        sb.append(remote_addr).append('\t');
        sb.append(paramsstr.substring(1, paramsstr.length()-1).replace(" ", "")).append('\t');
    ...
    public String toString() {
        ...
        sb.append(", ipprovince='").append(ipprovince).append('\'');
