...
public class ServerGroup {
     ...
     *
     * @param config config filename reader
     * @return group info
     */
    public static ServerGroup loadFromFile(ImmutableConfiguration config) throws Exception {
        ...
        for (int i = 0; i < group_count; i++) {
            ...
            for (int k = 0; k < szServers.length; k++) {
                ...
                if (parts.length != 2) {
                    throw new Exception("the value of item \"group" + i
                            + "\" is invalid, the correct format is host:port");
                }

                serverInfo = new ServerInfo(new InetSocketAddress(parts[0].trim(),
                ...
                if (server_index < 0) {
                    ...
                    if (serverGroup.servers.length > 0) {
                        System.arraycopy(serverGroup.servers, 0, servers, 0,
                                serverGroup.servers.length);
                    }
                    servers[serverGroup.servers.length] = serverInfo;
                    java.util.Arrays.sort(servers, compObj);
                    serverGroup.servers = servers;
                    server_index = java.util.Arrays.binarySearch(serverGroup.servers, serverInfo,
                            compObj);
                }
                ...
                if (java.util.Arrays.binarySearch(serverGroup.groups[i], serverInfo, compObj) < 0) {
                    ...
                    if (serverGroup.groups[i].length > 0) {
                        System.arraycopy(serverGroup.groups[i], 0, servers, 0,
                                serverGroup.groups[i].length);
                    }
    ...
    public boolean connectServer(ServerInfo server) {
        ...
        try {
            ...
            sock.connect(server.address, ClientGlobal.networkTimeout);
            server.sock = sock;
            return true;
    ...
    public void print() {
        ...
        for (i = 0; i < groups.length; i++) {
            ...
            for (int k = 0; k < groups[i].length; k++) {
                System.out.println("server" + (k + 1) + ". "
                        + groups[i][k].address.getAddress().getHostAddress() + ":"
