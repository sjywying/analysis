...
public class ProtoCommon {
        ...
        public static final int FDHT_EXPIRES_NONE = -1; // invalid timeout,
                                                        // should ignore

        public static final int FDHT_MAX_FULL_KEY_LEN = (FDHT_MAX_NAMESPACE_LEN + 1 + FDHT_MAX_OBJECT_ID_LEN + 1 + FDHT_MAX_SUB_KEY_LEN);
        ...
        public static PkgHeader recvHeader(InputStream in, byte expect_cmd, long expect_body_len) throws IOException {
                ...
                if (header.cmd != expect_cmd) {
                        throw new IOException("recv cmd: " + bs[PROTO_HEADER_CMD_INDEX]
                ...
                if (header.status == 0) {
                        if (expect_body_len >= 0 && header.body_len != expect_body_len) {
                                throw new IOException("recv body length: " + header.body_len
