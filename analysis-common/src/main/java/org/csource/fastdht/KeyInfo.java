...
public class KeyInfo {
    ...
    public KeyInfo(byte[] namespace, byte[] object_id, byte[] key) throws Exception {
    ...
    public KeyInfo(String namespace, String object_id, String key) throws Exception {
        this.namespace = namespace.getBytes(ClientGlobal.charset);
        this.object_id = object_id.getBytes(ClientGlobal.charset);
        this.key = key.getBytes(ClientGlobal.charset);
        this.check();
    }
    ...
    private void check() throws Exception {
        ...
        if ((namespace_len == 0 && obj_id_len != 0) || (namespace_len != 0 && obj_id_len == 0)) {
            throw new Exception("Invalid namespace length: " + this.namespace.length
                    + " and object ID length: " + this.object_id.length);
        }

        if (key_len == 0) {
            throw new Exception("Invalid key length: " + this.key.length);
        }
    }
    ...
    public int getHashCode() {
        ...
        key_hash_code = Hash.PJWHash(hash_key, 0, true_key_len);
        if (key_hash_code < 0) {
